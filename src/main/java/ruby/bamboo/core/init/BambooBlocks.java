...
public final class BambooBlocks {
    ...
    private static RegistryObject<MiniatureBlock> registerMiniature() {
        ...
        // デバッグ用 16x 敷き詰め (負荷計測用。後の負荷デバッグ用にクリエタブに常駐)
        BambooItems.addCreativeStack(() -> createFilledGlassMiniatureStack(block));
        BambooItems.addCreativeStack(() -> createFilledChestMiniatureStack(block));
        return block;
    }

    private static net.minecraft.world.item.ItemStack createFilledGlassMiniatureStack(RegistryObject<MiniatureBlock> block) {
        net.minecraft.world.item.ItemStack stack = new net.minecraft.world.item.ItemStack(block.get());
        int size = 16;
        ...
        bet.putInt(ruby.bamboo.block.entity.MiniatureBlockEntity.TAG_SIZE, size);
        net.minecraft.nbt.ListTag list = new net.minecraft.nbt.ListTag();
        net.minecraft.world.level.block.Block[] glasses = new net.minecraft.world.level.block.Block[]{
        ...
        for (int x = 0; x < size; x++) {
            for (int y = 0; y < size; y++) {
                for (int z = 0; z < size; z++) {
                    int idx = (x + y + z) % glasses.length;
                    // 位置で色をばらすことで視覚的に負荷デバッグしやすくする
                    net.minecraft.world.level.block.state.BlockState st = glasses[idx].defaultBlockState();
                    net.minecraft.nbt.CompoundTag entry = new net.minecraft.nbt.CompoundTag();
                    entry.putInt("X", x);
                    entry.putInt("Y", y);
                    entry.putInt("Z", z);
                    entry.put("State", net.minecraft.nbt.NbtUtils.writeBlockState(st));
                    list.add(entry);
                }
            }
        }
        bet.put(ruby.bamboo.block.entity.MiniatureBlockEntity.TAG_CELLS, list);
        tag.put(MiniatureBlock.BLOCK_ENTITY_TAG, bet);
        stack.setHoverName(net.minecraft.network.chat.Component.literal("ミニチュア (デバッグ: 色ガラス16x)"));
        return stack;
    }

    private static net.minecraft.world.item.ItemStack createFilledChestMiniatureStack(RegistryObject<MiniatureBlock> block) {
        net.minecraft.world.item.ItemStack stack = new net.minecraft.world.item.ItemStack(block.get());
        int size = 16;
        net.minecraft.nbt.CompoundTag tag = stack.getOrCreateTag();
        tag.putInt(ruby.bamboo.block.entity.MiniatureBlockEntity.TAG_SIZE, size);
        ...
        for (int x = 0; x < size; x++) {
            for (int y = 0; y < size; y++) {
                for (int z = 0; z < size; z++) {
                    ...
                    entry.putInt("Y", y);
                    entry.putInt("Z", z);
                    entry.put("State", net.minecraft.nbt.NbtUtils.writeBlockState(chestState));
                    list.add(entry);
                }
            }
        }
