...
public final class BambooItems {
    ...
    // ===== 宝石 3種 (旧 sakura_gem/ginkgo_gem + maple_ingot→改名。jem は gem の typo のため 2026-09-23 に戻し) =====
    // 旧 1.16.5 (sakura) では maple のみ ingot (製錬式) だったが、1.20.1 では
    // 3種とも gem 直ドロップ (幸運対応、製錬不要) に統一。用途は今後の刀・腕輪等の材料用。

    /** 紅葉宝石 (旧 maple_ingot から改名) */
    public static final RegistryObject<Item> MAPLE_GEM = register("maple_gem",
            () -> new Item(new Item.Properties()));
    /** 銀杏宝石 (旧 ginkgo_gem から改名) */
    public static final RegistryObject<Item> GINKGO_GEM = register("ginkgo_gem",
            () -> new Item(new Item.Properties()));
    /** 桜宝石 (旧 sakura_gem から改名) */
    public static final RegistryObject<Item> SAKURA_GEM = register("sakura_gem",
            () -> new Item(new Item.Properties()));

    /** バンブーエキス (竹9→1を囲炉裏で精製。遊び人Lv5取引の目玉用。テクスチャ仮置き) */
    public static final RegistryObject<Item> BAMBOO_EXTRACT = register("bamboo_extract",
            () -> new Item(new Item.Properties()));
