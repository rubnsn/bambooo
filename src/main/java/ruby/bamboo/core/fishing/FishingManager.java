 ...
 * データはハードコードのデフォルトテーブルを使用し、fishing/fish JSON があれば
 * FishingFishJsonLoader が差し替える。
 *
 * <p>1.21.1 NeoForge: ForgeRegistries → BuiltInRegistries、
 * new ResourceLocation → fromNamespaceAndPath、ランク付与は
 * DataComponents.CUSTOM_DATA 経由 (getOrCreateTag 撤去)。
 */
public final class FishingManager {
    ...
    private static List<FishingEntry> buildDefaultEntries() {
        ...
        list.add(FishingEntry.fish(
                ResourceLocation.fromNamespaceAndPath("bamboomod", "cod"),
                ...
                "is_ocean", 1.8f));
        list.add(FishingEntry.fish(
                ResourceLocation.fromNamespaceAndPath("bamboomod", "salmon"),
        ...
        // tropical は暖かい海で重み増
        list.add(FishingEntry.fish(
                ResourceLocation.fromNamespaceAndPath("bamboomod", "tropical_fish"),
                ...
                null, 1f));
        list.add(FishingEntry.fish(
                ResourceLocation.fromNamespaceAndPath("bamboomod", "pufferfish"),
        ...
        list.add(FishingEntry.junk(
                ResourceLocation.fromNamespaceAndPath("bamboomod", "junk_stick"),
                ResourceLocation.fromNamespaceAndPath("minecraft", "stick"), 3, 0));
        list.add(FishingEntry.junk(
                ResourceLocation.fromNamespaceAndPath("bamboomod", "junk_string"),
                ResourceLocation.fromNamespaceAndPath("minecraft", "string"), 3, 0));
        list.add(FishingEntry.junk(
                ResourceLocation.fromNamespaceAndPath("bamboomod", "junk_bone"),
                ResourceLocation.fromNamespaceAndPath("minecraft", "bone"), 2, 0));
        list.add(FishingEntry.junk(
                ResourceLocation.fromNamespaceAndPath("bamboomod", "junk_seaweed"),
                ResourceLocation.fromNamespaceAndPath("bamboomod", "seaweed"), 3, 0));
        // 宝箱 (TREASURE) - 木/鉄/金/ダイヤの金属価値で段階化。MVP では直接インゴット等を与える
        list.add(FishingEntry.treasure(
                ResourceLocation.fromNamespaceAndPath("bamboomod", "treasure_iron"),
                ResourceLocation.fromNamespaceAndPath("minecraft", "iron_ingot"), 2, 1));
        list.add(FishingEntry.treasure(
                ResourceLocation.fromNamespaceAndPath("bamboomod", "treasure_gold"),
                ResourceLocation.fromNamespaceAndPath("minecraft", "gold_ingot"), 1, 2));
        list.add(FishingEntry.treasure(
                ResourceLocation.fromNamespaceAndPath("bamboomod", "treasure_diamond"),
                ResourceLocation.fromNamespaceAndPath("minecraft", "diamond"), 1, 3));
     ...
     * @param random      乱数
     */
    public static RollResult roll(ServerPlayer player, net.minecraft.server.level.ServerLevel level, BlockPos pos,
    ...
    private static float getBiomeFactor(Level level, BlockPos pos, FishingEntry entry) {
        ...
        try {
            ...
            switch (entry.preferredBiomeTag) {
                ...
                case "is_warm_ocean" -> {
                    // warm ocean 系バイオームかをリソース位置で近似判定
    ...
    /**
     * 釣果の ItemStack を生成する。ランクは CUSTOM_DATA として付与する (FISH のみ)。
     */
    public static ItemStack createCatchStack(RollResult result, RandomSource random) {
        ...
        if (result.entry.category == FishingEntry.Category.FISH) {
            ...
            tag.putString(FishSize.TAG_KEY, result.size.tagValue);
