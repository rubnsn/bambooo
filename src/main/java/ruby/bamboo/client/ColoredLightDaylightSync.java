 ...
 * ({@code setSectionDirty} はSodium系も中継する標準経路)。
 * <p>
 * 1.21.1 NeoForge: 旧 Forge TickEvent.ClientTickEvent + Phase.END を
 * ClientTickEvent.Post に置換。
 */
    ...
    private static void onBucketChanged(ClientLevel level, Minecraft mc, int dirtyRadius) {
        ...
        for (int cx = pcx - renderChunks; cx <= pcx + renderChunks; cx++) {
            for (int cz = pcz - renderChunks; cz <= pcz + renderChunks; cz++) {
                ...
                if (!chunk.hasData(BambooCapabilities.COLORED_LIGHT.get())) continue;
