 ...
 * 1人称・手持ち描画には触らない。
 *
 * <p>1.21.1 NeoForge: イベントを NeoForge client パッケージ
 * (RenderPlayerEvent.Pre / ClientTickEvent.Post) へ移行し、EntityType 参照を
 * BuiltInRegistries へ移行。fake 実体の vanilla renderer 委譲方式のため、
 * 描画パイプライン (scale/ARGB/VertexConsumer) 側の変更は不要。
 * 廃止API (getPassengersRidingOffset 等) は使用していない。
 */
