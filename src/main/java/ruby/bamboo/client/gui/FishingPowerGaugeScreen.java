 ...
 *
 * <p>1.21.1 NeoForge: renderBackground は 4引数形式、
 * 送信は PacketDistributor.sendToServer。
 */
@OnlyIn(Dist.CLIENT)
public class FishingPowerGaugeScreen extends Screen {
    ...
    private void confirm() {
        ...
        try {
            PacketDistributor.sendToServer(new FishingCastRequestPacket(dist));
        } catch (Exception e) {
            e.printStackTrace();
        }
        Minecraft mc = Minecraft.getInstance();
    ...
    private void keepBowPull(Player player) {
        ...
        if (!player.isUsingItem()) {
            ...
            if (player.getOffhandItem().getItem() instanceof BambooRodItem)
