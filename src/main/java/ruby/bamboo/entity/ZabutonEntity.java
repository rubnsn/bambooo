...
public class ZabutonEntity extends Entity {
    ...
    protected void defineSynchedData() {
        this.entityData.define(DATA_COLOR, ZabutonColor.WHITE.ordinal());
    ...
    /**
     * 騎乗オフセット。旧は Entity 高さ由来で約+0.09だったため +0.1。
     */
    @Override
    ...
    public boolean hurt(DamageSource source, float amount) {
        ...
        if (source.getEntity() instanceof Player player) {
            ...
            if (!player.getAbilities().instabuild) {
                this.spawnAtLocation(new ItemStack(BambooItems.ZABUTONS.get(this.getColor().ordinal()).get()));
            }
            return true;
        }
        return false;
    ...
    public ItemStack getPickResult() {
        return new ItemStack(BambooItems.ZABUTONS.get(this.getColor().ordinal()).get());
    }
