 ...
 * <p>
 * 1.21 注記: 旧 HaC (dcs) 互換 ({@code DcsClimateCompat}) は未移植のため外した。
 * HaC 花・葉物の鉢植え状態解決が必要になったら互換層の移植後に再統合すること。
 */
public class FlowerBedBlockRenderer implements BlockEntityRenderer<FlowerBedBlockEntity> {
    ...
    public void render(FlowerBedBlockEntity be, float partialTick, PoseStack poseStack, MultiBufferSource buffer, int packedLight, int packedOverlay) {
        ...
        ItemRenderer ir = this.itemRenderer != null ? this.itemRenderer : mc.getItemRenderer();
        ...
        for (var e : plants) {
            ...
            if (e.stack.getItem() instanceof BlockItem bi) {
                ...
                renderPlantBlock(blockRenderer, plantState, poseStack, buffer, plantLight, packedOverlay, wx, wz, e.scale);
                continue;
            }
            poseStack.pushPose();
            boolean is3D = false;
