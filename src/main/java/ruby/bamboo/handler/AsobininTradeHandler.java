...
public final class AsobininTradeHandler {
    ...
    public static void onTrades(VillagerTradesEvent event) {
        ...
        Item bamboo = BambooItems.BAMBOO_EXTRACT.get();
        // Lv1: 竹 → mod 食料
        trades.get(1).add((trader, rand) -> offer(bamboo, 2, food("takeoni"), 2, 12, 2));
        trades.get(1).add((trader, rand) -> offer(bamboo, 2, food("takemesi"), 1, 12, 2));
        // Lv2-4: 竹 → 団子・丼・麺類
        trades.get(2).add((trader, rand) -> offer(bamboo, 3, food("dananko"), 2, 12, 10));
        trades.get(2).add((trader, rand) -> offer(bamboo, 3, food("kinooni"), 2, 12, 10));
        trades.get(3).add((trader, rand) -> offer(bamboo, 3, food("oyako"), 1, 12, 20));
        trades.get(3).add((trader, rand) -> offer(bamboo, 3, food("sakuramochi"), 1, 12, 20));
        trades.get(4).add((trader, rand) -> offer(bamboo, 4, food("katsudon"), 1, 12, 25));
        trades.get(4).add((trader, rand) -> offer(bamboo, 4, food("ramen"), 1, 12, 25));
        // Lv5 目玉: エキス64 → ガチャコイン・gem
        ...
        trades.get(5).add((trader, rand) -> offer(extract, 64, BambooItems.SAKURA_GEM.get(), 1, 2, 30));
        trades.get(5).add((trader, rand) -> offer(extract, 64, BambooItems.GINKGO_GEM.get(), 1, 2, 30));
        trades.get(5).add((trader, rand) -> offer(extract, 64, BambooItems.MAPLE_GEM.get(), 1, 2, 30));
    }
