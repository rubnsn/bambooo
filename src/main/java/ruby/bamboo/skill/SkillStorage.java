 ...
 *
 * <p>1.21.1 NeoForge: 永続化付き Player Attachment の値型。
 * {@code BambooCapabilities.SKILL} に
 * {@code AttachmentType.serializable(SkillStorage::new).copyOnDeath()} で登録する。
 * 旧 Forge の {@code INBTSerializable<CompoundTag>} (引数なし) とは異なり、
 * NeoForge の同名 IF は provider 付きのため両方実装する。
 * 実体は引数なし版で、sync パケット・クライアント反映からも呼ばれる。
 */
public class SkillStorage implements INBTSerializable<CompoundTag> {
    ...
    /** 実体。sync パケット生成・クライアント反映からも呼ばれる。 */
